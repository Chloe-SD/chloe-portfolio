...
const ProjectCard = ({ project, openModal }) => {
  ...
  return (
    <div className="bg-slate-900 bg-opacity-50 border-2 border-fuchsia-200 
      ...
      <div className="relative w-full pt-[56.25%] p-2"> {/* 16:9 aspect ratio */}
        <Image 
          src={thumbnail}
          alt={title}
          fill
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {tags?.slice(0, 3).map((tag, i) => (
            <span key={i} className="text-xs bg-pink-500 px-2 py-1 rounded-full text-white">
              {tag}
            </span>
          ))}
          {tags?.length > 3 && (
